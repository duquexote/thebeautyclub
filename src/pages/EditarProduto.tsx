...
export default function EditarProduto() {
  ...
  async function fetchProduto(produtoId: string) {
    try {
      ...
      setMessage({ text: `Erro ao buscar produto: ${error.message}`, type: 'error' });
    } finally {
  ...
  const handleSecao1ImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => {
        setSecao1ImagePreview(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSecao2ImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => {
        setSecao2ImagePreview(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSecao3ImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => {
        setSecao3ImagePreview(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 pt-24 max-w-2xl">
      <h1 className="text-3xl font-bold text-center mb-8 text-pink-600">Editar Produto</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6">
            ...
            {imagePreview && (
              <div className="mt-2">
                <p className="text-sm text-gray-600 mb-1">
                  {imageFile ? 'Nova imagem:' : 'Imagem atual:'}
                </p>
                <img 
                  src={imagePreview} 
                  alt="Preview" 
        ...
        <div className="flex items-center justify-between space-x-4">
          <button
            ...
            className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-1/2 transition duration-300"
          >
            Cancelar
          </button>
          <button
            type="submit"
            disabled={loading}
            className="bg-pink-500 hover:bg-pink-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-1/2 transition duration-300 disabled:opacity-50"
          >
